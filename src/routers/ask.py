...
async def _prepare_chunks_and_sources(
    ...
    with rag_tracer.trace_search(trace, request.query, request.top_k) as search_span:
        ...
        for idx, hit in enumerate(search_results.get("hits", []), 1):
            arxiv_id = hit.get("arxiv_id", "")
            chunk_text = hit.get("chunk_text", hit.get("abstract", ""))
            arxiv_id_clean = arxiv_id.split("v")[0] if "v" in arxiv_id else arxiv_id
            pdf_url = f"https://arxiv.org/pdf/{arxiv_id_clean}.pdf" if arxiv_id else ""
            ...
            chunks.append(
                {
                    ...
                    "chunk_index": idx,  # For citation numbering
                    "pdf_url": pdf_url,
                }
            )
...
async def ask_question(
    ...
    with rag_tracer.trace_request("api_user", request.query) as trace:
        try:
            ...
            if not chunks:
                from src.schemas.api.ask import ChunkInfo
            ...
            # Prepare chunk info for response
            from src.schemas.api.ask import ChunkInfo
            chunk_info_list = [
                ChunkInfo(
    ...
    async def generate_stream():
        ...
        with rag_tracer.trace_request("api_user", request.query) as trace:
            try:
                ...
                if cache_client:
                    try:
                        ...
                        if cached_response:
                            ...
                            # Convert chunks to dict format for streaming
                            chunks_dict = [
                                {
                            ...
                            ] if hasattr(cached_response, 'chunks') and cached_response.chunks else []
                ...
                # Prepare chunk info for streaming
                from src.schemas.api.ask import ChunkInfo
                chunk_info_list = [
                    {
                        "arxiv_id": chunk.get("arxiv_id", ""),
                ...
                # Send metadata first (including chunks)
                search_mode = "bm25" if not request.use_hybrid else "hybrid"
                metadata_response = {
                    "sources": sources,
                    "chunks": chunk_info_list,
