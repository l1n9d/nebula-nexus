...
class JinaEmbeddingsClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.jina.ai/v1", redis_client=None):
        ...
        self.client = httpx.AsyncClient(timeout=15.0)  # Reduced from 30s
        self.redis_client = redis_client
        self._embedding_cache_ttl = 86400  # 24 hours for embeddings
        logger.info("Jina embeddings client initialized with caching support")
    ...
    async def embed_query(self, query: str) -> List[float]:
        """Embed a search query with caching support.
        ...
        if self.redis_client:
            try:
                ...
                cache_key = f"embedding:query:{hashlib.md5(query.encode()).hexdigest()}"
                ...
                logger.warning(f"Cache read failed, proceeding with API call: {e}")
