...
from src.utils.retry import OLLAMA_RETRY, with_retry
...
class OllamaClient:
    ...
    async def warm_up_model(self, model: str = None) -> bool:
        """Warm up the model by sending a small prompt to preload it into memory.
        
        Args:
            model: Model name to warm up (uses default if not specified)
    ...
    async def generate_rag_answer_stream(
        ...
        try:
            ...
            # Stream the response with optimized parameters for speed
            async for chunk in self.generate_stream(
                model=model,
                prompt=prompt,
                temperature=0.3,  # Lower for faster, more focused responses
                top_p=0.85,  # Slightly lower for speed
                num_predict=512,  # Limit max tokens for faster generation
