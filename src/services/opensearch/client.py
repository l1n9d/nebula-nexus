...
from src.utils.retry import OPENSEARCH_RETRY, with_retry
...
class OpenSearchClient:
    ...
    def _create_rrf_pipeline(self, force: bool = False) -> bool:
        ...
        try:
            ...
            # RRF pipeline requires OpenSearch 2.17+, gracefully degrade for older versions
            logger.warning(f"RRF pipeline creation failed (likely OpenSearch version < 2.17): {e}")
            logger.warning("Hybrid search will work without RRF ranking, using score combination instead")
            return False
