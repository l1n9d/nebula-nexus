...
class PubMedSettings(BaseConfigSettings):
    model_config = SettingsConfigDict(
        env_file=[".env", str(ENV_FILE_PATH)],
        env_prefix="PUBMED__",
        extra="ignore",
    ...
    pdf_cache_dir: str = "./data/pubmed_pdfs"
    rate_limit_delay: float = 0.34  # ~3 requests/second (NCBI requirement without API key)
    timeout_seconds: int = 30
    max_results: int = 15
    search_term: str = "artificial intelligence[Title/Abstract] AND medical[Title/Abstract]"
    download_max_retries: int = 3
    download_retry_delay_base: float = 5.0
    max_concurrent_downloads: int = 5
    max_concurrent_parsing: int = 1
    
    # NCBI requires email and recommends API key
    email: str = "your.email@example.com"
    api_key: str = ""  # Optional but increases rate limit to 10 req/sec
    ...
    @classmethod
    def adjust_rate_limit_for_api_key(cls, v: float, info) -> float:
        # If API key is provided, allow faster requests (10/sec = 0.1s delay)
        # This will be overridden if explicitly set in env
        return v
...
class RedisSettings(BaseConfigSettings):
    ...
    # Cache settings
    ttl_hours: int = 12  # Cache TTL in hours (increased for better hit rate)
...
class Settings(BaseConfigSettings):
    ...
    postgres_pool_size: int = 10  # Optimized for 4 vCPU
    postgres_max_overflow: int = 10  # Allow burst capacity
